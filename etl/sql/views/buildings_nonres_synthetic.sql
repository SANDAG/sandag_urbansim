...
SELECT usp.parcel_id
	...
	,ref.[development_type_id] devtype_from_lu
--NOT GROUPED
	,emp.emp_adj emp
	,sqft.sqft_per_emp sqft_fromdev
	,sqft_fromlu.sqft_per_emp sqft_fromlu
--GROUPED
	--,SUM(emp_adj) emp
...
FROM urbansim.parcels usp
	...
	ON usp.luz_id = sqft_fromlu.luz_id AND ref.[development_type_id] = sqft_fromlu.development_type_id 
WHERE usp.parcel_id NOT IN (SELECT DISTINCT parcel_id FROM urbansim.buildings)							--NO BUILDING
AND usp.development_type_id != ref.[development_type_id]
