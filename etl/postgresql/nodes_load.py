...
SELECT node
      ...
      ,y
      ,on_ramp
FROM spacecore.urbansim.nodes
"""
##MSSQL SQLAlchemy
sql_in_engine = create_engine(in_connection_string)
##Pandas Data Frame for non-spatial data
df_non_spatial = pd.read_sql(in_query_non_spatial, sql_in_engine, index_col = 'node')
...
FROM spacecore.urbansim.nodes
"""
df_spatial = pd.read_sql(in_query_spatial, sql_in_engine, index_col='node')
